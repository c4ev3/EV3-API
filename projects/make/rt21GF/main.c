...
37grados/stud 
Small Cremallera max 1110grados
...
*/	
	...
	armAngle=MoveArmTimeProtected(my_robot.ArmAMotorPort, 80, -300, 4000,true);
    TurnGyro(0,30,30);
	//Ahora vamos a por las pesas
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(450),0,60,10,20,true,false);
	StraightbyGyroDegreesToUmbral(my_robot.ColorLeftPort,0,20,10, true);
	StraightbyGyroDegrees(CalculateTravelDegrees(10),0,10,true,true);
	TurnGyro(-90,30,30);
	TurnGyro(-90,30,30);
	armAngle=MoveArmTimeProtected(my_robot.ArmBMotorPort, -80, 1100, 4000,true);
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(115),-90,60,10,10,true,true);
	TurnGyro(-90,30,30);
	waitButton();
	// Esto es la vuelta
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(450),0,-60,-10,-15,true,true); 
...
//	StraighLAGDegrees(CalculateTravelDegrees(600), my_robot.ColorLeftPort, 50, 0, 40, EDGELEFT, true);
	...
	TurnGyro(-90,40,30);
    //StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(300),-90,75,5,60,true,false);
	StraightbyGyroDegreesToUmbral(my_robot.ColorLeftPort, -90,30,80,false);
	StraightbyGyroDegreesWithBrake(CalculateTravelDegrees(220), -90, 60, 5, true, true); //llegada a Boccia //260-Toledo 230-try1
	// Aqui empieza el rocknroll
	MoveArmTimeProtected(my_robot.ArmAMotorPort, 100, 525, 2000, true); //Calculates ratio por 63 grados de apertura del switch neumatico
	ResetCounterMotor(my_robot.ArmAMotorPort);
...
int SalidaLarguero(){
	...
	//veersion sin contador de pasos
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(200),0,75,10,40,true,false);
	TurnGyro(-15,40,30);
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(450),-15,75,10,40,true,false);
	TurnGyro(0,40,30);
	TurnGyro(0,40,30);
	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(250),0,60,10,10,true,false);
	TurnGyro(-90,40,30);
	TurnGyro(-90,40,30);
...
int SalidaCarmen_GF(){
	...
	SetLightPID(0.1,0.1,3.0);
  	...
  	StraightbyGyroDegreesWithRamps(CalculateTravelDegrees(1120),0,75,20,10,true,true); //1080
	//StraightbyGyroDegreesWithBrake(CalculateTravelDegrees(1120),0,20,10,false,true);//1110 Toledo
	/* Bajamos volteador, muñeco*/
    int brazo = MoveArmTimeProtected(my_robot.ArmAMotorPort, -100, 1700, 5000, true); //
//	LcdTextf(1, 0, LcdRowToY(1), "arm_fin...%d",brazo);
	// Wait(2000);
    Wait(500);
...
int Aux3() {
	...
	ResetPowerCounters();
	SetTimerMS(1,0); 
	angulo = TurnGyroFast(-90, 10, 2);
	Wait(2000);
	angulo = TurnGyrowithClearance(0, 40, 30, 1);	
	...
	*/
	...
	LcdTextf(1, 0, LcdRowToY(2), "Angulo: %d",angulo);
